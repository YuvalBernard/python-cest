...
from simulate_spectra import (
    batch_gen_MTR_Assym_analytical,
    batch_gen_MTR_Assym_numerical,
    batch_gen_MTR_Assym_ss_analytical,
    batch_gen_MTR_Assym_ss_direct_symbolic,
    batch_gen_MTR_Assym_ss_numerical,
    batch_gen_MTR_Assym_symbolic,
)
...
class DataPage(QWizardPage):
    def __init__(self):
        ...
        self.convert_to_mtr_button = QPushButton("Convert Z-spectra to MTR_Assym")
        self.convert_to_mtr_button.clicked.connect(self.convert_to_mtr)
        ...
        self.layout.addWidget(self.convert_to_mtr_button, alignment=Qt.AlignmentFlag.AlignLeft)
        ...
        self.graphWidget.setLabel("left", "MTR_Assym [a.u.]")
        self.graphWidget.setLabel("bottom", "offset [ppm]")

    def convert_to_mtr(self):
        data = self.wizard().data
        x_mtr, mtr_Assym = Z_to_MTR_Assym(data.iloc[:, 0].to_numpy(), data.iloc[:, 1:].T.to_numpy())
        self.wizard().data = pd.DataFrame(np.c_[x_mtr, mtr_Assym.T], columns=data.columns)
        self.updateUi()
    ...
    def normalizeData(self):
        ...
        selected_option, ok = QInputDialog.getItem(
            self,
            "Normalize Data",
            "Select offset by which to normalize data",
            self.wizard().data["ppm"].astype("string").to_list(),
            editable=False,
        )
        if ok:
            idx = self.wizard().data.loc[self.wizard().data["ppm"] == float(selected_option)].index
            for col in self.wizard().data.columns[1:]:
                self.wizard().data[col] = self.wizard().data[col].apply(lambda x: x / self.wizard().data[col][idx])
            self.updateUi()
        else:
            return
...
class SolverGroup(QGroupBox):
    ...
    def validateSolver(self):
        if self.parent.field("solver") in {"Analytical", "Analytical Steady State"}:  # Analytical
...
class ResultPage(QWizardPage):
    ...
    def perform_fit(self):
        ...
        match self.field("solver"):
            case "Symbolic":
                self.solver = batch_gen_MTR_Assym_symbolic
            case "Symbolic Steady State":
                self.solver = batch_gen_MTR_Assym_ss_direct_symbolic
            case "Analytical":
                self.solver = batch_gen_MTR_Assym_analytical
            case "Analytical Steady State":
                self.solver = batch_gen_MTR_Assym_ss_analytical
            case "Numerical":
                self.solver = batch_gen_MTR_Assym_numerical
            case "Numerical Steady State":
                self.solver = batch_gen_MTR_Assym_ss_numerical
