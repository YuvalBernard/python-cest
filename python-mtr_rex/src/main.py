...
class DataPage(QWizardPage):
    def __init__(self):
        ...
        self.convert_to_mtr_button = QPushButton("Convert Z-spectra to MTR_REX")
        ...
        self.graphWidget.setLabel("left", "MTR_REX [a.u.]")
        self.graphWidget.setLabel("bottom", "offset [ppm]")

    def convert_to_mtr(self):
        data = self.wizard().data
        x_mtr, mtr_rex = Z_to_MTR_REX(data.iloc[:, 0].to_numpy(), data.iloc[:, 1:].T.to_numpy())
        self.wizard().data = pd.DataFrame(np.c_[x_mtr, mtr_rex.T], columns=data.columns)
...
class SolverGroup(QGroupBox):
    def __init__(self, parent: QWizardPage):
        ...
        solver_list.addItems(
            [
                "Symbolic",
                "Symbolic Steady State (Direct)",
                "Symbolic Steady State (Indirect)",
                "Analytical",
...
class ResultPage(QWizardPage):
    ...
    def perform_fit(self):
        ...
        match self.field("solver"):
            case "Symbolic":
                self.solver = batch_gen_MTR_REX_symbolic
            case "Symbolic Steady State (Direct)":
                self.solver = batch_gen_MTR_REX_ss_direct_symbolic
            case "Symbolic Steady State (Indirect)":
                self.solver = batch_gen_MTR_REX_ss_indirect_symbolic
            case "Analytical":
                self.solver = batch_gen_MTR_REX_analytical
            case "Analytical Steady State":
                self.solver = batch_gen_MTR_REX_ss_analytical
            case "Numerical":
                self.solver = batch_gen_MTR_REX_numerical
            case "Numerical Steady State":
                self.solver = batch_gen_MTR_REX_ss_numerical
