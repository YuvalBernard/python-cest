...
def solve_bloch_mcconnell_3_pools():
    (
        ...
        power,
        tp,
    ) = sp.symbols("R1a, R2a, R1b, R2b, R1c, R2c,  kb, kc,  fb, fc,  offset, dwa, dwb, dwc,  power, tp", real=True)
    kab = fb * kb
    kac = fc * kc
    L_a = sp.Matrix(
        [
    ...
    with open("../lib/solve_bloch_mcconnell_3_pools.py", "w") as text_file:
        text_file.write("import jax.numpy as jnp\n")
        text_file.write(inspect.getsource(gen_spectrum))

    filereplace("../lib/solve_bloch_mcconnell_3_pools.py", "_lambdifygenerated", "gen_spectrum_symbolic")
    filereplace("../lib/solve_bloch_mcconnell_3_pools.py", "exp", "jnp.exp")
    filereplace("../lib/solve_bloch_mcconnell_3_pools.py", "abs", "jnp.absolute")
    filereplace("../lib/solve_bloch_mcconnell_3_pools.py", "sqrt", "jnp.sqrt")
